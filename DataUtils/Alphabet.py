...
class CreateAlphabet:
    ...
    def __init__(self, min_freq=1, train_data=None, dev_data=None, test_data=None, config=None):
        ...
        self.accu_label_state = collections.OrderedDict()
        self.law_label_state = collections.OrderedDict()
        ...
        self.accu_label_alphabet = Alphabet()
        self.law_label_alphabet = Alphabet()
    ...
    def build_vocab(self):
            ...
            for word in data.words:
                ...
                if type(word) == list:
                    ...
                    if word not in self.word_state:
                        ...
                        self.word_state[word] += 1
            # accu label
            for label in data.accu_labels:
                if label not in self.accu_label_state:
                    self.accu_label_state[label] = 1
                else:
                    self.accu_label_state[label] += 1

            # law label
            for label in data.law_labels:
                if label not in self.law_label_state:
                    self.law_label_state[label] = 1
                else:
                    self.law_label_state[label] += 1
        ...
        self.accu_label_alphabet.initialWord2idAndId2Word(self.accu_label_state)
        self.law_label_alphabet.initialWord2idAndId2Word(self.law_label_state)
        ...
        self.accu_label_alphabet.set_fixed_flag(True)
        self.law_label_alphabet.set_fixed_flag(True)
