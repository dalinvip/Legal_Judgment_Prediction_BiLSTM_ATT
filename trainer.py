...
class Train(object):
        ...
        for epoch in range(1, epochs + 1):
            ...
            for batch_count, batch_features in enumerate(self.train_iter):
                ...
                if (steps - 1) % self.config.log_interval == 0:
                    ...
                    F1_measure(accu, batch_features.accu_label_features, self.accu_train_eval_micro, self.accu_train_eval_macro, cuda=self.config.use_cuda)
                    (accu_p_avg, accu_r_avg, accu_f_avg), _, _ = getFscore_Avg(self.accu_train_eval_micro, self.accu_train_eval_macro, accu.size(1))
                    sys.stdout.write(
                        "\nbatch_count = [{}/{}] , total_loss is {:.6f}, [accu-ACC is {:.6f}%]".format(batch_count + 1, self.train_iter_len, total_loss.data[0], accu_p_avg))
            end_time = time.time()
    ...
    @staticmethod
    def _eval_batch(data_iter, model, accu_eval_micro, accu_eval_macro, law_eval_micro, law_eval_macro,
        ...
        for batch_count, batch_features in enumerate(data_iter):
            ...
            F1_measure(accu, batch_features.accu_label_features, accu_eval_micro, accu_eval_macro, cuda=config.use_cuda)
            F1_measure(law, batch_features.law_label_features, law_eval_micro, law_eval_macro, cuda=config.use_cuda)

        # get f-score
        # accu_p, accu_r, accu_f = getFscore_Avg(accu_eval_micro, accu_eval_macro, accu.size(1))
        # law_p, law_r, law_f = getFscore_Avg(law_eval_micro, law_eval_macro, law.size(1))
